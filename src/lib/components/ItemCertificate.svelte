<script lang="ts">
  ...
  export let data;
  export let myneCards = data.getMyneCards;
  ...
  let swiperEl: Swiper;
  afterUpdate(() => {
    swiperEl = document.getElementById("item-certificate-slider").swiper;
...
<div class=" w-full h-[80vh] flex flex-col ">
  ...
  <div class="item-certificate-area flex  w-full h-full  rounded-2xl px-2">
    <div
      class="item-certificate-slider-container min-h-[80%]  flex h-full w-[60%] bg-slate-100 shadow-md rounded-2xl my mx-1 relative"
    >
      <swiper-container
        id="item-certificate-slider"
        bind:this={itemCertificateSwiperEl}
        transition:fade={{ duration: 100 }}
        class="item-certificate-slider w-full  min-h-[85%]  flex bg-slate-100 shadow-md rounded-2xl my mx-1"
        navigation="true"
        pagination="true"
        effect="coverflow"
        grab-cursor="true"
      ...
      >
        ...
        {:then Module}
          {#key cardsFiltered}
            {#each cardsFiltered as myneCard}
              <swiper-slide class="w-fit h-fit " transition:fly|local={{ duration: 200, delay: 300 }}>
    ...
    <div class="item-certificate-controls h-full w-[40%]  bg-slate-200 rounded-2xl  mx-1 shadow-lg flex flex-col">
      ...
      <div class="w-full h-fit flex justify-center ">
        <button class="btn btn-primary generate-certificate w-[90%] normal-case"> Generate Certificate </button>
      </div>
    </div>
  </div>
</div>
