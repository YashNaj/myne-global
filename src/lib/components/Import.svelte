...
<div class="wrapper w-full h-full flex flex-col " transition:fade|local>
  ...
  >
   ...
   -->
      {#if !guideOpen}
        <div>
          <h1 class="guide-heading font-semibold text-2xl p-2 italic">Import Guide</h1>
          <p class="w-full py-1 px-4 text-sm">
            Before you can import your inventory to Myne Global, you'll have to correctly format your document to match
            the fields that Myne Global collects. <br />
            <span class="font-semibold "> This guide is in Beta, values are subject to change. </span>
          </p>
        </div>
      {/if}
      <div class="category-select flex flex-col relative w-full justify-start py-1 px-4">
        {#if !guideOpen}
      ...
      {#if guideOpen && selectedCategory != undefined}
        ...
        >
          <div class="w-full">
            ...
            <p class="text-sm">
              You can map your values as you wish to Myne Global's general structure, this will ensure painless
              importing into our database, as well as generate the most complete cards. None of these fields are
              required, though it is recommended to map values as completely as possible. For example <span
                class="font-bold"
              >
                puchaseValue
              </span>
              can become the <span class="font-bold">MSRP </span>of your product.
            </p>
          </div>
          <div
          ...
          >
            <table class="table w-full" transition:slide|local={{ delay: 200, duration: 200, easing: quintOut }}>
              <!-- head -->
              <thead>
                <tr>
                  <th />
          ...
          <div class="code-box instructions w-full mt-2">
            ...
            <p class="text-sm">
              To make transfer simple for advanced users, we have provided an array of the properties. Feel free to use
              your preferred methodology to map values programatically.
            </p>
          </div>
          <div class="code-box mt-2 rounded-2xl card bg-priamry">
            <div class=" text-sm p-3  w-fit h-fit">
              <pre class="text-sm" data-prefix=">"><code>const {selectedCategory.toLowerCase()} = [</code></pre>
              {#each tableProps as prop}
                <pre class="text-sm" data-prefix=">"><code>"{prop}",</code></pre>
              {/each}
          ...
          <div class="template instructions w-full mt-2">
            ...
            <p class="text-sm">
              Download a template of the specified category, and copy and paste your columns into the template, this is
              not recommended but the option is yours.
            </p>
          </div>
      ...
      >
        {#if guideOpen}
          <Icon src={Minus} class="text-primary transition ease-in-out hover:scale-[130%]" size="22px" />
        {:else}
          <Icon src={Plus} class="text-primary transition ease-in-out hover:scale-[130%]" size="22px" />
        {/if}
      </button>
    </div>
    <label>
      ...
      <div
        class="md:flex-1 h-[35vh] md:mt-0 w-full mt-4 bg-gradient-to-r from-bg-slate-300 to-bg-slate-500 hover:bg-slate-500 group hover:transition-colors 
           rounded-2xl flex flex-wrap justify-center content-center
          [box-shadow:_rgba(50,_50,_93,_0.25)0px_30px_60px-12px_inset,_rgba(0,_0,_0,_0.3)0px_18px_36px-18px_inset;]"
        bind:this={dropZone}
      >
